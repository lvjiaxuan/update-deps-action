...
function main() {
  const tool = core.getInput("tool", { required: false });
  const email = core.getInput("email", { required: true });
  (0, import_child_process.execSync)(`npx ${tool === "ncu" ? "npm-check-updates -u" : "taze -w -r"}`);
  const gitStatus = (0, import_child_process.execSync)("git status").toString();
  if (!gitStatus.includes("working tree clean")) {
    core.info("Find new dependency(ies).\n");
    (0, import_child_process.execSync)("corepack enable");
    (0, import_child_process.execSync)("npm i @antfu/ni -g");
    (0, import_child_process.execSync)("ni --lockfile-only --frozen-lockfile=false");
    (0, import_child_process.execSync)(`git config user.name ${github.context.actor}`);
    (0, import_child_process.execSync)(`git config user.email ${email} }`);
    (0, import_child_process.execSync)("git add .");
    (0, import_child_process.execSync)('git commit -m "chore: update dependency."');
